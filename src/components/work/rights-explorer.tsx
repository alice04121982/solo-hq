import { CircleSlash, ExternalLink, Scale } from "lucide-react";
 ...
 * the whole point of the component: a reader who acts on a blur between
 * statutory rights and employer policy is the person this page exists for.
 *
 * One entry only, the UK. The panel sits on a white card, because the lime
 * fill that marks "this is the law" only holds its edge against white.
 */
export function RightsExplorer({ entry }: { entry: RightsEntry }) {
  return (
    <article
      id={entry.slug}
...
function LawPanel({ points }: { points: RightsPoint[] }) {
  return (
    <div className="rounded-2xl p-5 md:p-6" style={{ background: "var(--accent)" }}>
...
function NotLawPanel({ points }: { points: RightsPoint[] }) {
  return (
    ...
    <div className={`flex flex-wrap gap-2 ${className ?? ""}`}>
      {sources.map((s) => (
        <a
          ...
          className="inline-flex items-center gap-1.5 rounded-full bg-background px-3.5 py-1.5 text-[13px] font-sans transition-colors hover:bg-[var(--teal)] hover:text-white text-teal"
        >
          {s.label}
