...
interface Phase {
  ...
  title: string;
  subtitle: string;
  /** Source file preserved for future use — not rendered in this component */
  illustration: string;
...
const PHASES: Phase[] = [
  {
    ...
    illustrationAlt: "Brain with a lightbulb — the moment of realisation",
    description:
  ...
  },
  {
    ...
    illustrationAlt: "Hands nurturing a small plant growing from someone's head — cultivating your foundations",
    description:
    ...
    actions: [
      ...
      "Set a budget ceiling and a 'stop' point before you start",
  ...
  },
  {
    ...
    illustration: "/images/illustrations/phase-donor-hunt.png",
    illustrationAlt: "Microscope with DNA strand — researching genetics",
    description:
  ...
  },
  {
    ...
    illustration: "/images/illustrations/phase-clinic.png",
    illustrationAlt: "Online consultation on a phone — researching and connecting with clinics",
    description:
  ...
  },
  {
    ...
    illustrationAlt: "Female scientist in a lab with test tubes — the science of treatment",
    description:
  ...
  },
  {
    ...
    illustrationAlt: "Therapist with a patient — counselling and support",
    description:
  ...
  },
  {
    ...
    illustrationAlt: "Pregnant woman surrounded by nature — growing your family",
    description:
  ...
  },
  {
    ...
    illustration: "/images/illustrations/phase-birth.png",
    illustrationAlt: "Sleepy bear under the moon — the beautiful exhaustion of early parenthood",
    description:
  ...
  },
  {
    ...
    illustrationAlt: "A flourishing brain with plants growing — thriving and growing as a solo family",
    description:
...
export function JourneyMap() {
  const [openId, setOpenId] = useState<string>("decision");
  ...
  function toggle(id: string) {
    ...
    if (isOpening) {
      ...
      if (el) {
        ...
        // Scroll only when the row is below 120px from viewport top or above the fold
        if (rect.top > 120 || rect.top < 0) {
          window.scrollTo({
            top: Math.max(0, window.scrollY + rect.top - 100),
            behavior: "smooth",
          });
        }
      }
  ...
  return (
    <section className="py-16 md:py-24 px-6 md:px-10 lg:px-[50px]">
      {/* Single consolidated heading block */}
      <div className="mb-12 md:mb-14">
        <p className="text-[11px] font-[500] uppercase tracking-[0.18em] text-muted mb-4 font-sans">
          Solo Navigator
        </p>
        <h2
      ...
      <div>
        {PHASES.map((phase) => {
          ...
          const panelId = `solo-nav-panel-${phase.id}`;
          const triggerId = `solo-nav-trigger-${phase.id}`;

          return (
            <div key={phase.id} className="border-t border-border">
              {/* Row trigger button */}
              <button
                id={triggerId}
                ref={(el) => {
                  buttonRefs.current[phase.id] = el;
                }}
                onClick={() => toggle(phase.id)}
                aria-expanded={isOpen}
                aria-controls={panelId}
                className="w-full flex items-center gap-5 md:gap-8 py-6 md:py-7 text-left rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 transition-colors duration-150"
                style={
                  {
              ...
              >
                ...
                <div className="flex-1 min-w-0 flex flex-col sm:flex-row sm:items-baseline sm:gap-4">
                  ...
                  <span className="text-[15px] font-sans text-muted mt-0.5 sm:mt-0 leading-relaxed">
                    {phase.subtitle}
                  </span>
                </div>

                {/* Chevron — green, rotates on expand */}
                <ChevronDown
              ...
              {/* Expandable panel — grid-template-rows animation for smooth height */}
              <div
                id={panelId}
                role="region"
              ...
              >
                {/* overflow:hidden on inner wrapper is required for the 0fr → 1fr trick */}
                <div className="overflow-hidden">
                  <div
                    className="pb-8 max-w-2xl"
                    /* Indent to align with the title column: number width + gap */
                    style={{ paddingLeft: "calc(2.25rem + 1.25rem)" }}
                  >
                    {/* Body paragraph */}
                    ...
                    <ul className="space-y-3 list-none" role="list">
                      {phase.actions.map((action) => (
                        <li key={action} className="flex items-start gap-3">
                          <span
                            className="h-1.5 w-1.5 rounded-full shrink-0 mt-[0.6em]"
                            style={{ background: "var(--accent)" }}
                            aria-hidden="true"
                          />
                          <span className="text-[15px] font-sans text-muted leading-relaxed">
                            {action}
                          </span>
                        </li>
                      ))}
